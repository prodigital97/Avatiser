...
.g-sound { position:absolute; bottom:10px; right:10px; width:34px; height:34px; display:flex; align-items:center; justify-content:center; background:rgba(8,8,8,.72); border:1px solid var(--border2); border-radius:50%; color:var(--white2); cursor:pointer; backdrop-filter:blur(6px); -webkit-backdrop-filter:blur(6px); transition:background .2s var(--ease), transform .2s var(--ease); z-index:2; padding:0; }
.g-sound:hover { background:rgba(8,8,8,.92); transform:scale(1.06); }
.g-sound .ic-on { display:none; }
.g-sound.is-on .ic-off { display:none; }
.g-sound.is-on .ic-on { display:block; }
  ...
  var io = new IntersectionObserver(function (es) {
    es.forEach(function (e) {
      if (e.isIntersecting) {
        ...
        if (!v.muted) {
          v.muted = true;
          var fig = v.closest('.g-item');
          var btn = fig && fig.querySelector('.g-sound');
...
var soundBtns = Array.prototype.slice.call(document.querySelectorAll('.g-sound'));
soundBtns.forEach(function (btn) {
  var fig = btn.closest('.g-item');
  ...
  btn.addEventListener('click', function () {
    ...
    soundBtns.forEach(function (other) {
      if (other === btn) return;
      var oFig = other.closest('.g-item');
      var oVid = oFig && oFig.querySelector('video');
      if (oVid) oVid.muted = true;
...
const SOUND_ICON_OFF = `<svg class="ic-off" viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="22" y1="9" x2="16" y2="15"></line><line x1="16" y1="9" x2="22" y2="15"></line></svg>`;
const SOUND_ICON_ON = `<svg class="ic-on" viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M15.54 8.46a5 5 0 0 1 0 7.07"></path><path d="M18.36 5.64a9 9 0 0 1 0 12.73"></path></svg>`;

const mediaHtml = (m) => m.type === 'video'
  ? `<figure class="g-item"><span class="g-tag">${m.tag}</span><video data-src="${m.src}" ${m.poster ? `poster="${m.poster}"` : ''} muted loop playsinline preload="none"></video><button class="g-sound" type="button" aria-label="toggle sound" aria-pressed="false">${SOUND_ICON_OFF}${SOUND_ICON_ON}</button></figure>`
